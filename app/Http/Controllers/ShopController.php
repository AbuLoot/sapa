...
class ShopController extends Controller
{
    public function index()
    {
        ...
        $modes = Mode::whereIn('slug', ['new', 'top'])->first();
    ...
    public function projectProducts(Request $request, $project_slug, $project_id)
    {
        ...
        $products_all = $query->whereHas('projects', function ($query_project) use ($ids) {
                    $query_project->whereIn('project_id', $ids);
                })->get();
        ...
        if ($request->has('project_id')) {
            ...
            $query->whereHas('projects', function ($query_project) use ($project_id) {
                    $query_project->whereIn('project_id', $project_id);
                });
        }
        else {
            $query->whereHas('projects', function ($query_project) use ($ids) {
                    $query_project->whereIn('project_id', $ids);
                });
        }
        ...
        $options = DB::table('products')
            ...
            ->whereIn('category_id', $categories_ids_boot)
    ...
    public function categoryProducts(Request $request, $category_slug, $category_id)
    {
        ...
        $projects_category = DB::table('products')
            ->join('product_project', 'products.id', '=', 'product_project.product_id')
            ->join('projects', 'projects.id', '=', 'product_project.project_id')
            ->select('projects.id', 'projects.slug', 'projects.title')
