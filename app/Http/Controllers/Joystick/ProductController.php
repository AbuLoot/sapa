...
class ProductController extends Controller
{
    ...
    public function copyImgs($lang)
    {
        ...
        foreach ($products as $product) {
            ...
            if (Storage::exists('img/products/'.$product->image)) {

                Storage::copy('img/products/'.$product->image, 'img/products2/'.$dirName.'/'.$product->image);

                $images = unserialize($product->images) ?? [''];

                foreach ($images as $key => $image) {

                    if (Storage::exists('img/products/'.$product->path.'/'.$images[$key]['image'])) {
                        Storage::move('img/products/'.$images[$key]['image'], 'img/products2/'.$dirName.'/'.$images[$key]['image']);
                        Storage::move('img/products/'.$images[$key]['present_image'], 'img/products2/'.$dirName.'/'.$images[$key]['present_image']);
                    }
                }

                echo $i++ . $product->title . '<br><img src="/img/products2/'.$dirName.'/'.$product->image.'"><hr>';
            } else {
                $n++;
            }
        }
    ...
    public function setProject($lang)
    {
        $products = Product::all();
        $projects = Project::get();

        foreach($products as $product) {
            if ($projects->contains($product->project_id)) {
                $product->projects()->sync($product->project_id);
            }
        }
    ...
    public function setCount($lang)
    {
        // DB::table('products')
            // ->where('products.id', '=', 'products.id')
            // ->update(['id_codes' => json_encode('id_code')]);
        // dd(1234);

        $storeId = Store::select('id')->first();
        ...
        foreach($products as $product) {
            ...
            $sum = array_sum($idCodes);
            $countInStores[$id] = $sum;
    ...
    public function setCode($lang)
    {
        ...
        // foreach($products as $product) {
        //     $product->id_codes = json_encode([$product->id_code]);
        //     $product->save();
        // }
